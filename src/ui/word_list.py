...
class WordList(ctk.CTkFrame):
    ...
    def unbind_keys(self):
        ...
        self.master.unbind_all("<Key-j>")        # Global navigation keys are handled by App class, no need to unbind here
    ...
    def update_content(self, new_content):
        ...
        # Add empty string as first element to allow navigation to first word
        ...
        # Start with no word highlighted since first element is empty
        # User will navigate to first real word using 'l' key
    ...
    def change_choosen_word(self, event, is_forward: bool):
        ...
        self.highlight_word_at_position(self.choosen_word_pos)
          # Throttled TTS - only play if enough time has passed and word is not empty
        ...
        if current_time - self.last_tts_time >= self.tts_delay:
            if self.word_list and self.choosen_word_pos < len(self.word_list):
                ...
                # Only play TTS if the word is not empty
    ...
    def setup_text_tags(self):
        ...
        # Disable editing again        self.content_textbox.configure(state="disabled")
    ...
    def highlight_word_at_position(self, word_position):
        ...
        # Skip highlighting if the word is empty (position 0)
        if word_position == 0 and self.word_list[word_position] == "":
            self.content_textbox.configure(state="normal")
        ...
        # Calculate the character position of the word (skip the first empty element)
        char_position = 0
        for i in range(1, word_position):  # Start from 1 to skip empty first element
    ...
    def navigate_word_detail(self, event):
        # Don't navigate if current word is empty or invalid
